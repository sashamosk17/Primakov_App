...
class MockRatingRepository {
    ...
    initializeTestData() {
        ...
        teacher1Ratings.forEach((rating, index) => {
            this.ratings.push(new Rating_1.Rating(`rating-teacher1-${index + 1}`, "teacher-1", rating.userId, rating.value, new Date(Date.now() - (10 - index) * 24 * 60 * 60 * 1000), rating.comment));
        });
        ...
        teacher2Ratings.forEach((rating, index) => {
            this.ratings.push(new Rating_1.Rating(`rating-teacher2-${index + 1}`, "teacher-2", rating.userId, rating.value, new Date(Date.now() - (12 - index) * 24 * 60 * 60 * 1000), rating.comment));
        });
    }
