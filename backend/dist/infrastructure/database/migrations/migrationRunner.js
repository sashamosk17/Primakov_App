...
async function runMigrations(pool) {
    ...
    try {
        ...
        const lockResult = await lockClient.query("SELECT pg_try_advisory_lock($1) as acquired", [lockId]);
    ...
    `);
        ...
        const executedResult = await lockClient.query("SELECT name FROM migrations ORDER BY name");
        ...
        for (const file of files) {
            ...
            await executeWithRetry(async () => {
                ...
                const sql = fs_1.default.readFileSync(filePath, "utf-8").replace(/^\uFEFF/, "");
